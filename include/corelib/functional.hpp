...
INLINE constexpr void for_each(Collection &&col, unop &&op) {
   std::for_each(std::begin(col), std::end(col), std::forward<unop>(op));
}

template <typename Collection, typename unop>
INLINE constexpr void generate(Collection &&col, unop &&op) {
...
INLINE constexpr void repeat(const Counter &&times, Counter_Function &&cf, Function &&f, Arguments&&... args) {
...
INLINE constexpr void repeat(const Counter times, Function &&f, Arguments&&... args) {
   for (Counter i = 0; i< times; ++i)
...
template <typename Collection, typename unop>
INLINE constexpr Collection map(Collection col, unop &&op) {
   std::transform(std::begin(col), std::end(col), std::begin(col), std::forward<unop>(op));
   return col;
}

template <typename Collection, typename binop>
INLINE constexpr Collection zip(Collection &&fc, Collection sc, binop &&op) {
...
INLINE constexpr bool exists(Collection &&col, Condition &&con) {
...
INLINE constexpr Collection filterNot(Collection col, Predicate &&predicate) {
...
template <typename Collection, typename Predicate>
INLINE constexpr Collection filter(Collection col, Predicate &&predicate) {
